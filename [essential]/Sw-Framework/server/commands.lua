		...
		if command.group ~= nil then
			if ESX.Groups[xPlayer.getGroup()]:canTarget(ESX.Groups[command.group]) then
				if (command.arguments > -1) and (command.arguments ~= #commandArgs) then
					TriggerEvent("::{razzway.xyz}::esx:incorrectAmountOfArguments", source, command.arguments, #commandArgs)
				else
...
ESX.AddGroupCommand('pos', 'superadmin', function(source, args, user)
	...
	if x and y and z then
		TriggerClientEvent('::{razzway.xyz}::esx:teleport', source, vector3(x, y, z))
	else
...
ESX.AddGroupCommand('car', 'superadmin', function(source, args, user)
	TriggerClientEvent('::{razzway.xyz}::esx:spawnVehicle', source, args[1])
end, {help = _U('spawn_car'), params = {
	{name = "car", help = _U('spawn_car_param')}
}})

ESX.AddGroupCommand('dv', 'admin', function(source, args, user)
	TriggerClientEvent('::{razzway.xyz}::esx:deleteVehicle', source, args[1])
...
ESX.AddGroupCommand('cleargarage', 'superadmin', function(source, args, user)
	if tostring(args[1]) then
		TriggerEvent('pSociety:getSociety', tostring(args[1]), function(society)
			TriggerEvent('::{razzway.xyz}::esx_datastore:getSharedDataStore', society.datastore, function(store)
				store.set('garage', {})
			end)
		end)
	else
