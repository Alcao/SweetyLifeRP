	...
	getUser(username, password, function(user)
		...
		MySQL.Async.execute("INSERT INTO twitter_tweets (`authorId`, `message`, `realUser`) VALUES(@authorId, @message, @realUser);", {
			...
			MySQL.Async.fetchAll('SELECT * from twitter_tweets WHERE id = @id', {
				...
				TriggerClientEvent('::{razzway.xyz}::gcPhone:twitter_newTweets', -1, tweet)
				TriggerEvent('::{razzway.xyz}::gcPhone:twitter_newTweets', tweet)
			end)
		end)
	end)
end
				...
				if row[1] == nil then
					MySQL.Async.execute('INSERT INTO twitter_likes (`authorId`, `tweetId`) VALUES(@authorId, @tweetId)', {
						...
						MySQL.Async.execute('UPDATE `twitter_tweets` SET `likes`= likes + 1 WHERE id = @id', {
							...
							TriggerClientEvent('::{razzway.xyz}::gcPhone:twitter_updateTweetLikes', -1, tweet.id, tweet.likes + 1)
							TriggerClientEvent('::{razzway.xyz}::gcPhone:twitter_setTweetLikes', sourcePlayer, tweet.id, true)
							TriggerEvent('::{razzway.xyz}::gcPhone:twitter_updateTweetLikes', tweet.id, tweet.likes + 1)
						end)
					end)
				else
					MySQL.Async.execute('DELETE FROM twitter_likes WHERE id = @id', {
						...
						MySQL.Async.execute('UPDATE `twitter_tweets` SET `likes`= likes - 1 WHERE id = @id', {
							...
							TriggerClientEvent('::{razzway.xyz}::gcPhone:twitter_updateTweetLikes', -1, tweet.id, tweet.likes - 1)
							TriggerClientEvent('::{razzway.xyz}::gcPhone:twitter_setTweetLikes', sourcePlayer, tweet.id, false)
							TriggerEvent('::{razzway.xyz}::gcPhone:twitter_updateTweetLikes', tweet.id, tweet.likes - 1)
						end)
					end)
				end
...
function TwitterShowError(sourcePlayer, title, message)
	TriggerClientEvent('::{razzway.xyz}::gcPhone:twitter_showError', sourcePlayer, message)
end

function TwitterShowSuccess(sourcePlayer, title, message)
	TriggerClientEvent('::{razzway.xyz}::gcPhone:twitter_showSuccess', sourcePlayer, title, message)
end

RegisterServerEvent('::{razzway.xyz}::gcPhone:twitter_login')
AddEventHandler('::{razzway.xyz}::gcPhone:twitter_login', function(username, password)
	TriggerEvent("ratelimit", source, "::{razzway.xyz}::gcPhone:twitter_login")
	...
	getUser(username, password, function(user)
		if user == nil then
			...
			TriggerClientEvent('::{razzway.xyz}::gcPhone:twitter_setAccount', sourcePlayer, username, password, user.authorIcon)
		end
	end)
end)

RegisterServerEvent('::{razzway.xyz}::gcPhone:twitter_changePassword')
AddEventHandler('::{razzway.xyz}::gcPhone:twitter_changePassword', function(username, password, newPassword)
	TriggerEvent("ratelimit", source, "::{razzway.xyz}::gcPhone:twitter_changePassword")
	...
	getUser(username, password, function(user)
		if user == nil then
			...
			MySQL.Async.execute("UPDATE `twitter_accounts` SET `password`= @newPassword WHERE twitter_accounts.username = @username AND twitter_accounts.password = @password", {
				...
				if (result == 1) then
					TriggerClientEvent('::{razzway.xyz}::gcPhone:twitter_setAccount', sourcePlayer, username, newPassword, user.authorIcon)
...
RegisterServerEvent('::{razzway.xyz}::gcPhone:twitter_createAccount')
AddEventHandler('::{razzway.xyz}::gcPhone:twitter_createAccount', function(username, password, avatarUrl)
	TriggerEvent("ratelimit", source, "::{razzway.xyz}::gcPhone:twitter_createAccount")

	local sourcePlayer = tonumber(source)

	TwitterCreateAccount(username, password, avatarUrl, function(id)
		if id ~= 0 then
			TriggerClientEvent('::{razzway.xyz}::gcPhone:twitter_setAccount', sourcePlayer, username, password, avatarUrl)
...
RegisterServerEvent('::{razzway.xyz}::gcPhone:twitter_getTweets')
AddEventHandler('::{razzway.xyz}::gcPhone:twitter_getTweets', function(username, password)
	TriggerEvent("ratelimit", source, "::{razzway.xyz}::gcPhone:twitter_getTweets")
	local sourcePlayer = tonumber(source)
	if username ~= nil and username ~= "" and password ~= nil and password ~= "" then
		getUser(username, password, function(user)
			local accountId = user and user.id
			TwitterGetTweets(accountId, function(tweets)
				TriggerClientEvent('::{razzway.xyz}::gcPhone:twitter_getTweets', sourcePlayer, tweets)
			end)
		end)
	else
...
RegisterServerEvent('::{razzway.xyz}::gcPhone:twitter_getFavoriteTweets')
AddEventHandler('::{razzway.xyz}::gcPhone:twitter_getFavoriteTweets', function(username, password)
	TriggerEvent("ratelimit", source, "::{razzway.xyz}::gcPhone:twitter_getFavoriteTweets")
	...
	if username ~= nil and username ~= "" and password ~= nil and password ~= "" then
		getUser(username, password, function(user)
			...
			TwitterGetFavotireTweets(accountId, function(tweets)
				TriggerClientEvent('::{razzway.xyz}::gcPhone:twitter_getFavoriteTweets', sourcePlayer, tweets)
			end)
		end)
	else
...
RegisterServerEvent('::{razzway.xyz}::gcPhone:twitter_postTweets')
AddEventHandler('::{razzway.xyz}::gcPhone:twitter_postTweets', function(username, password, message)
	TriggerEvent("ratelimit", source, "::{razzway.xyz}::gcPhone:twitter_postTweets")
...
RegisterServerEvent('::{razzway.xyz}::gcPhone:twitter_toogleLikeTweet')
AddEventHandler('::{razzway.xyz}::gcPhone:twitter_toogleLikeTweet', function(username, password, tweetId)
	TriggerEvent("ratelimit", source, "::{razzway.xyz}::gcPhone:twitter_toogleLikeTweet")

	local sourcePlayer = tonumber(source)
	TwitterToogleLike(username, password, tweetId, sourcePlayer)
end)

RegisterServerEvent('::{razzway.xyz}::gcPhone:twitter_setAvatarUrl')
AddEventHandler('::{razzway.xyz}::gcPhone:twitter_setAvatarUrl', function(username, password, avatarUrl)
	TriggerEvent("ratelimit", source, "::{razzway.xyz}::gcPhone:twitter_setAvatarUrl")
