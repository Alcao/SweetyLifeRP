...
function OpenMenu(submitCb, cancelCb, restrict)
	...
	TriggerEvent('::{razzway.xyz}::skinchanger:getData', function(components, maxVals)
		local elements = {}
		local _components = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'skin', {
			...
			DeleteSkinCam()
			TriggerEvent('::{razzway.xyz}::skinchanger:loadSkin', LastSkin)
			...
			TriggerEvent('::{razzway.xyz}::skinchanger:getSkin', function(skin)
				...
				if skin[data.current.name] ~= data.current.value then
					TriggerEvent('::{razzway.xyz}::skinchanger:change', data.current.name, data.current.value)

					TriggerEvent('::{razzway.xyz}::skinchanger:getData', function(_, maxVals)
						for i = 1, #elements, 1 do
							local newData = {}
							newData.max = maxVals[elements[i].name]

							if elements[i].textureof ~= nil and data.current.name == elements[i].textureof then
								TriggerEvent('::{razzway.xyz}::skinchanger:change', elements[i].name, 0)
								newData.value = 0
							end
