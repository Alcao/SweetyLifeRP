...
Citizen.CreateThread(function()
	...
	for _,playerId in ipairs(players) do
		...
		MySQL.Async.fetchAll('SELECT status FROM users WHERE identifier = @identifier', {
			...
			TriggerClientEvent('::{razzway.xyz}::esx_status:load', playerId, data)
		end)
	end
end)
...
AddEventHandler('::{razzway.xyz}::esx_status:getStatus', function(playerId, statusName, cb)
	local xPlayer = ESX.GetPlayerFromId(playerId)
	local status  = xPlayer.get('status')
...
RegisterServerEvent('::{razzway.xyz}::esx_status:update')
AddEventHandler('::{razzway.xyz}::esx_status:update', function(status)
...
ESX.AddGroupCommand('heal', 'admin', function(source, args, user)
	if tonumber(args[1]) then
		...
		TriggerClientEvent('::{razzway.xyz}::esx_status:healPlayer', source)
	end
