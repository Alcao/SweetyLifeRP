...
AddEventHandler("astra_staff:takeReport", function(reportId)
    ...
    if not reportsTable[reportId] then
        TriggerClientEvent("::{razzway.xyz}::esx:showNotification", source, "~r~[Report] ~s~Ce report n'est plus en attente de prise en charge")
        return
    end
    reportsTable[reportId].takenBy = GetPlayerName(source)
    reportsTable[reportId].taken = true
    if players[reportId] ~= nil then
        TriggerClientEvent("::{razzway.xyz}::esx:showNotification", reportId, "~r~[Report] ~s~Votre report a été pris en charge.")
    end
...
AddEventHandler("astra_staff:closeReport", function(reportId)
    ...
    if not reportsTable[reportId] then
        TriggerClientEvent("::{razzway.xyz}::esx:showNotification", source, "~r~[Report] ~s~Ce report n'est plus valide")
        return
    end
    if players[reportId] ~= nil then
        TriggerClientEvent("::{razzway.xyz}::esx:showNotification", reportId, "~r~[Report] ~s~Votre report a été cloturé. N'hésitez pas à nous recontacter en cas de besoin.")
    end
    ...
    for k, player in pairs(players) do
        if player.rank ~= "user" then
            if inService[k] ~= nil then
                TriggerClientEvent("::{razzway.xyz}::esx:showNotification", k, message)
            end
        end
    end
end
...
RegisterCommand("report", function(source, args)
    ...
    if reportsTable[source] ~= nil then
        TriggerClientEvent("::{razzway.xyz}::esx:showNotification", source, "~r~[Report] ~s~Vous avez déjà un report actif.")
        return
    end
    ...
    TriggerClientEvent("::{razzway.xyz}::esx:showNotification", source, "~r~[Report] ~s~Votre report a été envoyé ! Vous serez informé quand il sera pris en charge et / ou cloturé.")
    updateReportsForStaff()
end, false)
