...
AddEventHandler('VmLife:DeleteTenue', function(id,label)
  ...
  TriggerClientEvent("::{razzway.xyz}::esx:showNotification",source,"Tenue supprimé")
...
AddEventHandler('VmLife:RenameTenue', function(id,label)
  ...
  TriggerClientEvent("::{razzway.xyz}::esx:showNotification",source,"Vous avez bien renommé votre tenue en "..label)
...
AddEventHandler("Mushy:RenameMasque", function(id,txt,type)
  ...
  TriggerClientEvent("::{razzway.xyz}::esx:showNotification",source,"Vous avez bien renommé votre "..type.." en "..txt)
...
AddEventHandler('Mushy:Delclo', function(id,label,data)
	...
	local xPlayer = ESX.GetPlayerFromId(_source)
      ...
      TriggerClientEvent('::{razzway.xyz}::esx:showNotification', _source, '~r~-1 '.. label)
...
AddEventHandler("Mushy:SetNewMasque", function(mask,variation,type,label)
 ...
 -- if label == nil then
  ...
  TriggerClientEvent("Mushy:SyncAccess",source)
  TriggerClientEvent("::{razzway.xyz}::esx:showNotification",source,"~g~Vous avez reçu un nouveau " .. type .."~n~~r~-50$" )
else
  TriggerClientEvent('::{razzway.xyz}::esx:showNotification', _source, 'Pas assez d\'argent (50$)')
end
end)
...
AddEventHandler('Mushy:GiveAccessories', function(target,id,label)
	...
	MySQL.Async.execute(
		'UPDATE user_accessories SET identifier = @identifier WHERE id = @id',
		{
			...
			TriggerClientEvent('::{razzway.xyz}::esx:showNotification', _source, '~r~-1 '.. label)
			TriggerClientEvent('::{razzway.xyz}::esx:showNotification', target, '~g~+1 '.. label)
...
AddEventHandler('shop:price', function(prix)
  ...
  TriggerClientEvent('Alcao:check', source)
	TriggerClientEvent('::{razzway.xyz}::esx:showNotification', source, "~y~Shop~w~ : Vous avez payer 25$ ! ")
end)