...
-- Load the default player skin (for ::{razzway.xyz}::esx_skin)
...
function HandleMarkers()
	...
	if CurrentStatus ~= Status.DELIVERY_INACTIVE then
		DrawMarker(20, deleter.x, deleter.y, deleter.z, 0, 0, 0, 0, 180.0, 0, 1.5, 1.5, 1.5, 255, 0, 62, 150, true, true)
		...
		if CurrentStatus == Status.PLAYER_STARTED_DELIVERY then
			if not IsPlayerInsideDeliveryVehicle() and CurrentVehicle ~= nil then
				...
				DrawMarker(20, VehiclePos.x, VehiclePos.y, ArrowHeight, 0, 0, 0, 0, 180.0, 0, 0.8, 0.8, 0.8, 255, 0, 62, 150, true, true)
			else
				local dl = DeliveryLocation.Item1
				if GetDistanceBetweenCoords(pCoords.x, pCoords.y, pCoords.z, dl.x, dl.y, dl.z, true) < 150 then
					DrawMarker(20, dl.x, dl.y, dl.z, 0, 0, 0, 0, 180.0, 0, 1.5, 1.5, 1.5, 255, 0, 62, 150, true, true)
				end
			end
		end
		...
		if CurrentStatus == Status.PLAYER_REACHED_VEHICLE_POINT then
			if not IsPlayerInsideDeliveryVehicle() then
				...
				DrawMarker(20, TrunkPos.x, TrunkPos.y, TrunkHeight, 0, 0, 0, 180.0, 0, 0, ArrowSize.x, ArrowSize.y, ArrowSize.z, 255, 0, 62, 150, true, true)
				
				if GetDistanceBetweenCoords(pCoords.x, pCoords.y, pCoords.z, TrunkPos.x, TrunkPos.y, TrunkHeight, true) < 1.0 then
					DisplayHelpText("Appuyez sur ~INPUT_CONTEXT~ pour retirer votre ~y~marchandise~s~ de votre véhicule")
					if IsControlJustReleased(0, 51) then
						PlayTrunkAnimation()
		...
		if CurrentStatus == Status.PLAYER_REMOVED_GOODS_FROM_VEHICLE then
			...
			DrawMarker(20, dp.x, dp.y, dp.z, 0, 0, 0, 0, 180.0, 0, 1.5, 1.5, 1.5, 255, 0, 62, 150, true, true)
		end
		...
		if GetDistanceBetweenCoords(pCoords.x, pCoords.y, pCoords.z, bCoords.x, bCoords.y, bCoords.z, true) < 50.0 then
			...
			if GetDistanceBetweenCoords(pCoords.x, pCoords.y, pCoords.z, ScooterPos.x, ScooterPos.y, ScooterPos.z, true) < 1.5 then
				DisplayHelpText("Appuyez sur ~INPUT_CONTEXT~ pour demarrer les livraisons, dépôt de garantie ~b~$"..tostring(Config.Safe.scooter))
				SelectType = 'scooter'
