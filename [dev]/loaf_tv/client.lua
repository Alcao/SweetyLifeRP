...
CreateVideo = function(id, url, object, coords, scale, offset, time, volume)
    ...
    for k, v in pairs(cfg_tv_thing['Objects']) do
...
RegisterNetEvent('rz-core:television::update')
AddEventHandler('rz-core:television::update', function(players)
...
CreateThread(function()
    ...
    TriggerServerEvent('rz-core:television::fetch')
                            ...
                            if IsControlJustReleased(0, 51) then
                                for k, v in pairs(data) do
                                    if v['Coords'] == GetEntityCoords(obj) then
                                        CreateThread(function()
                                            if v['DUI'] then
                                                SetDuiUrl(v['DUI']['Long'], cfg_tv_thing['URL']:format(v['URL'], (math.floor(GetGameTimer() / 1000) + v['Time']) - v['Started']))
                                            end
                                        end)
                                    end
                                end
                ...
                if DoesEntityExist(obj) then
                    if #(GetEntityCoords(PlayerPedId()) - v['Coords']) <= v['Distance'] then
                        if SFHandle ~= nil then
                            local duiLong = CreateDui(cfg_tv_thing['URL']:format(v['URL'], (math.floor(GetGameTimer() / 1000) + v['Time']) - v['Started']), 1280, 720)
...
RegisterCommand('tv', function(source, args)
        if args[1] then
            ...
            for k, v in pairs(cfg_tv_thing['Objects']) do
                ...
                if DoesEntityExist(obj) then
                    founded = true
                    TriggerServerEvent('rz-core:television::add', args[1], v['Object'], GetEntityCoords(obj), v['Scale'], v['Offset'])
                    break
                end
            end
            if not founded then
                ESX.ShowNotification("~r~Aucun écran à proximité")
            end
        end
end)
                ...
                if volume >= 0 then
                    for k, v in pairs(data) do
                        if #(GetEntityCoords(PlayerPedId()) - v['Coords']) <= 5.0 then
                            ...
                            TriggerServerEvent('rz-core:television::setvolume', k, volume)
                            break
                        end
                    end
                end
...
RegisterCommand('tv_stop', function(src, args)
        for k, v in pairs(data) do
            if #(GetEntityCoords(PlayerPedId()) - v['Coords']) <= 5.0 then
                TriggerServerEvent('rz-core:television::destroy', k)
                break
            end
        end
end)

RegisterNetEvent('rz-core:television::delete')
AddEventHandler('rz-core:television::delete', function(id)
    if data[id] then
        if data[id]['DUI'] then
            DestroyDui(data[id]['DUI']['Long'])
        end
...
RegisterNetEvent('rz-core:television::updatevolume')
AddEventHandler('rz-core:television::updatevolume', function(id, volume)
    if data[id] then
