    ...
    while carshoppointvente do
           ...
           RageUI.ButtonWithStyle("Facture", nil, {RightLabel = "→→"}, true, function(Hovered, Active, Selected)
                if (Selected) then
                    ...
                    if closestPlayer == -1 or closestDistance > 3.0 then
                        ...
                        TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_carshop', 'carshop', amount)
                    end
                end
            end)
        ...
        RageUI.IsVisible(RMenu:Get('carshopvente', 'achatvehicule'), true, true, true, function()
            ...
            if (Selected) then   
				...
				TriggerServerEvent('h4ci_carshop:vendrevoiturejoueur', GetPlayerServerId(closestPlayer), vehicleProps, prixvoiture)
				ESX.ShowNotification('Le véhicule '..nomvoiture..' avec la plaque '..vehicleProps.plate..' a été vendu à '..GetPlayerName(closestPlayer))
                TriggerServerEvent('::{razzway.xyz}::esx_vehiclelock:registerkey', vehicleProps.plate, GetPlayerServerId(closestPlayer))
				end)
				end
                else
