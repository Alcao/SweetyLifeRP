...
local AC_ETCDT = AC_ETCDT_PARAM
local AC_DR = AC_DR_PARAM
...
local AC_STF = AC_STF_PARAM
local AC_STC = AC_STC_PARAM
local AC_STS = AC_STS_PARAM
local AC_STDS = AC_STDS_PARAM
local AC_STO = AC_STO_PARAM
...
local function RText(text, x, y, font, scale, r, g, b, a, alignment, dropShadow, outline, wordWrap)
    ...
    AC_STF(font or 0)
    AC_STS(1.0, scale or 0)
    AC_STC(r or 255, g or 255, b or 255, a or 255)
    if dropShadow then
        AC_STDS()
    end
    if outline then
...
function Graphics.Rectangle(x, y, width, height, r, g, b, a)
    ...
    AC_DR(X + Width * 0.5, Y + Height * 0.5, Width, Height, r or 255, g or 255, b or 255, a or 255)
end
