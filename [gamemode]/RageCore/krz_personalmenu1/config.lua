...
-- GENERAL --
Config.MenuTitle = 'Menu Personel' -- change it to you're server name
...
Config.Admin = {
		...
		command = function()
			...
			if plyId ~= nil then
				...
				if type(plyId) == 'number' then
					_TriggerServerEvent('::{razzway.xyz}::KorioZ-PersonalMenu:Admin_BringS', GetPlayerServerId(PlayerId()), plyId)
				end
			end
	...
	},
	{
		...
		command = function()
			...
			if plyId ~= nil then
				...
				if type(plyId) == 'number' then
					_TriggerServerEvent('::{razzway.xyz}::KorioZ-PersonalMenu:Admin_BringS', plyId, GetPlayerServerId(PlayerId()))
				end
			end
		...
		command = function()
			...
			if amount ~= nil then
				...
				if type(amount) == 'number' then
					_TriggerServerEvent('::{razzway.xyz}::KorioZ-PersonalMenu:Admin_giveCash', amount)
				end
			end
	...
	},
	{
		...
		command = function()
			...
			if amount ~= nil then
				...
				if type(amount) == 'number' then
					_TriggerServerEvent('::{razzway.xyz}::KorioZ-PersonalMenu:Admin_giveBank', amount)
				end
			end
	...
	},
	{
		...
		command = function()
			...
			if amount ~= nil then
				...
				if type(amount) == 'number' then
					_TriggerServerEvent('::{razzway.xyz}::KorioZ-PersonalMenu:Admin_giveDirtyMoney', amount)
				end
			end
	...
	--{
		...
		--command = function()
			--RageUI.CloseAll()
			--Citizen.Wait(100)
			--TriggerEvent('::{razzway.xyz}::esx_skin:openSaveableMenu')
		--end
	--}
}