...
Citizen.CreateThread(function()
	...
	while true do
		...
		DisableControlAction(0, ConfigKRZ.Controls.Crouch.keyboard, true)

		if IsDisabledControlJustReleased(0, ConfigKRZ.Controls.Crouch.keyboard) and IsInputDisabled(2) then
		...
		if IsControlJustReleased(0, ConfigKRZ.Controls.HandsUP.keyboard) and IsInputDisabled(2) then
			if DoesEntityExist(plyPed) and not IsEntityDead(plyPed) and IsPedOnFoot(plyPed) then
		...
		if IsControlJustReleased(0, ConfigKRZ.Controls.Pointing.keyboard) and IsInputDisabled(2) then
			if DoesEntityExist(plyPed) and not IsEntityDead(plyPed) and IsPedOnFoot(plyPed) then
				if Player.handsUp then
