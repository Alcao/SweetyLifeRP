...
SwLife.newThread(function()
    while true do
        Citizen.Wait(300000)
        SwLife.InternalToServer("XNL_NET:AddPlayerXP", 600) 
    end 
end)
...
local disabledSafeZonesKeys = {
    ...
    {group = 0, key = 140, message = '~r~Vous devez être Niveau 3 pour pouvoir faire ceci.'},
                ...
                if not ohunflic then
                    ...
                    if IsDisabledControlJustPressed(disabledSafeZonesKeys[i].group, disabledSafeZonesKeys[i].key) then
                        ...
                        if disabledSafeZonesKeys[i].message then
                            ESX.ShowNotification('~b~XP~s~ - ~y~Niveau insuffisant\n~w~Vous devez être au niveau 3 pour pouvoir effectuer cette action.')
                        end
                    end
                end
            end
