...
function LoadDefaultModel(malePed, cb)
	...
	Citizen.CreateThread(function()
		ESX.Streaming.RequestModel(characterModel, function()
			...
			TriggerEvent('::{razzway.xyz}::skinchanger:modelLoaded')
		end)
	end)
end
...
AddEventHandler('::{razzway.xyz}::skinchanger:loadDefaultModel', function(loadMale, cb)
	LoadDefaultModel(loadMale, cb)
end)

AddEventHandler('::{razzway.xyz}::skinchanger:getData', function(cb)
...
AddEventHandler('::{razzway.xyz}::skinchanger:change', function(key, val)
	Character[key] = val

	if key == 'sex' then
		TriggerEvent('::{razzway.xyz}::skinchanger:loadSkin', Character)
	else
		ApplySkin(Character)
	end
end)

AddEventHandler('::{razzway.xyz}::skinchanger:getSkin', function(cb)
	cb(Character)
end)
...
RegisterNetEvent('::{razzway.xyz}::skinchanger:loadSkin')
AddEventHandler('::{razzway.xyz}::skinchanger:loadSkin', function(skin, cb)
	if skin['sex'] ~= LastSex then
		LoadSkin = skin

		if skin['sex'] == 0 then
			TriggerEvent('::{razzway.xyz}::skinchanger:loadDefaultModel', true, cb)
		else
			TriggerEvent('::{razzway.xyz}::skinchanger:loadDefaultModel', false, cb)
		end
	else
		ApplySkin(skin)
...
RegisterNetEvent('::{razzway.xyz}::skinchanger:loadClothes')
AddEventHandler('::{razzway.xyz}::skinchanger:loadClothes', function(playerSkin, clothesSkin)
	if playerSkin['sex'] ~= LastSex then
		...
		if playerSkin['sex'] == 0 then
			TriggerEvent('::{razzway.xyz}::skinchanger:loadDefaultModel', true)
		else
			TriggerEvent('::{razzway.xyz}::skinchanger:loadDefaultModel', false)
		end
	else
