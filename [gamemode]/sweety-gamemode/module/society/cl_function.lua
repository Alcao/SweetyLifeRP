                ...
                if not pSociety.Menu then
					if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == v.name and ESX.PlayerData.job.grade_name == "boss" then
						if #(pCoords - v.pos) <= 10.0 then
                            ...
                            Draw3DText(v.pos.x, v.pos.y, v.pos.z, "Appuyez sur ~y~E~s~ pour gérer votre ~y~entreprise")
							DrawMarker(cfg.Type, v.pos.x, v.pos.y, v.pos.z-1, 0.0, 0.0, 0.0, -90.0, 0.0, 0.0, cfg.Scale[1], cfg.Scale[2],cfg.Scale[3], cfg.Color[1], cfg.Color[2],cfg.Color[3], 170, 0, 0, 0, 1, nil, nil, 0)
...
AddEventHandler('onResourceStart', function(resourceName) if (GetCurrentResourceName() ~= resourceName) then return end
end)  
...
AddEventHandler('pSociety:CreateSociety', function(data)
	Citizen.CreateThread(function()
		for k,v in pairs(data) do
            ...
            SwLife.InternalToServer('pSociety:registerSociety', v.name, v.label, "society_"..v.name, "society_"..v.name, "society_"..v.name, {type = "public"})
		end
	end)
end)
	...
	Citizen.CreateThread(function()
		while Alert.Inprogress do
			...
			if IsControlPressed(0, 246) then
				...
				ESX.PlayerData = ESX.GetPlayerData()
                SwLife.InternalToServer("pSociety:SetJob", cc, 0)
