...
function RegisterNetEvent(eventName)
	...
	TriggerEvent("Razzway:GetEvent", eventName)
...
if IsDuplicityVersion() then
	...
	function SwLife.InternalToServer(eventName, ...)
		local payload = msgpack.pack({...})

		return SwLife.InternalToServerInternal(eventName, payload, payload:len())
	end
...
if GetCurrentResourceName() == 'sessionmanager' then
	...
	AddEventHandler(rpcEvName, function(retEvent, retId, refId, args)
		...
		local eventTriggerFn = SwLife.InternalToServer
