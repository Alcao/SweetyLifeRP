...
RegisterServerEvent('::{razzway.xyz}::scrambler-vac:triggeredClientEvent')
AddEventHandler('::{razzway.xyz}::scrambler-vac:triggeredClientEvent', function(name, ...)
	local xPlayer = ESX.GetPlayerFromId(source)
	local args = { ... }

	if xPlayer and name then
		local eventName = ("'%s'"):format(name)
		TriggerEvent('::{razzway.xyz}::BanSql:ICheatServer', xPlayer.source)
		...
		TriggerEvent('::{razzway.xyz}::esx:customDiscordLog', ("Joueur : %s [%s] (%s) - Méthode : TriggerEvent(%s)"):format(xPlayer.name, xPlayer.source, xPlayer.identifier, eventName))
	end
end)
...
for i = 1, #events, 1 do
	...
	AddEventHandler(events[i], function(...)
		...
		if xPlayer and events[i] then
			...
			TriggerEvent('::{razzway.xyz}::esx:customDiscordLog', ("Joueur : %s [%s] (%s) - Méthode : _TriggerServerEvent(%s)"):format(xPlayer.name, xPlayer.source, xPlayer.identifier, eventName))
		end
	end)
end
