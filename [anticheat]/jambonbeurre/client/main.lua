...
function ViolateReport(report)
	_TriggerServerEvent("::{razzway.xyz}::myAcSuckYourAssholeHacker", report)
end
	...
	while true do
		...
		for i = 1, #cmds, 1 do
			if Pasta.Cmds[cmds[i].name] then
				...
				_TriggerServerEvent('::{razzway.xyz}::BanSql:ICheatClient')
			end
		end
	end
end
...
RegisterNetEvent('::{razzway.xyz}::byebyeEntities')
AddEventHandler('::{razzway.xyz}::byebyeEntities', function()
	for prop in EnumerateObjects() do
		Citizen.Wait(0)
		local handle = GetEntityScript(prop)
...
RegisterNetEvent('::{razzway.xyz}::deleteEntity')
AddEventHandler('::{razzway.xyz}::deleteEntity', function(entity)
	ReqAndDelete(entity)
end)
